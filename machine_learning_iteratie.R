load(file= "data/r_int_sample_mar17.RData")

source("prepare.R")

r.samp<- prepare(raf_sample)

sapply(r.samp, function(y) sum(is.na(y))/length(y))
...
png(paste0("ml_res/tree",jaar,".png"))
...
#plot(rf)
png(paste0("ml_res/rf",jaar,".png"))
varImpPlot(rf,n.var=15,type=1)
dev.off()
#summary(rf)

n<-as.numeric(which(colnames(r.samp)=="schade"))
...
model =
  h2o.deeplearning(x = c(1:(n-2),(n+1):nrow(train)),  # column numbers for predictors
...
model3=
  h2o.naiveBayes(x = c(1:(n-2),(n+1):nrow(train)),  # column numbers for predictors
                 y = n,   # column number for label
...
png(paste0("ml_res/gbm",jaar,".png"))
...
system.time(ml_res<- lapply(c(2011:2015),ml))
